...
int main ( int argc, char *argv[] ) {
    ...
    if (ProcRank == 0) {                            // 0 процесс (рассылка и принятие)
        ...
        for (int i=0; i<10; ++i) {
            MPI_Send(&byte_array, n, MPI_BYTE, 1, 0, MPI_COMM_WORLD);
            MPI_Recv(&byte_array, n, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &Status);
        }
        ...
        for (int i=0; i<10; ++i) {
            ...
            MPI_Recv(&my_enother_u_int8_t_array, n, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &Status);
            MPI_Send(&my_enother_u_int8_t_array, n, MPI_BYTE, 0, 0, MPI_COMM_WORLD);
        }
    }
    MPI_Finalize();
    return 0;
